...
- **Hooked FLOPs abandoned 2026-09-13 05:50 (user: GPU/time to the final goals).** The bf16-twin
  CPU run loaded 203 GB and then printed nothing for 3 h at 1250% CPU (bf16 MoE forward on CPU,
  4k-token prompts; 120 forwards needed) -- killed. GLM-4.6V's FLOPs column uses the closed form
  (`flops_analytic.Glm46VDecoder/Glm46VVision`, unit-tested against hand counts, A12B check to 6
  decimals) and the table note says so. If a hooked reconciliation is ever wanted: the bf16 twin
  under HF `device_map="auto"` across two GPUs on B200-8, with the `Glm4vMoeTextExperts` hook.
