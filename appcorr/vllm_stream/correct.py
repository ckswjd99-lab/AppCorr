...
Design memo: ``docs/memo/vllm_interleaved_design.md`` (§2, §7.11).  Deviations from it are listed
in ``install``'s docstring.
"""
...
def open_buffer(req_id: str, n: int, device, *, lo: int = 0, hi: int = 0,
                capture_out: bool = False) -> SideBuffer:
...
def appcorr_rows_step(self: GPUModelRunner, req_id: str, positions: torch.Tensor, layers,
    ...
    cap = min(int(sched.max_num_seqs), int(sched.max_num_batched_tokens))
...
def appcorr_staged_correct(self: GPUModelRunner, req_id: str, positions: torch.Tensor,
                           ...
                           replay: bool = False, fuse: bool = False) -> dict:
    """Depth-staged round ``stage = (r, g)``: correct ``positions`` over ``[0, b_r)``, then walk
    every image row ``[lo, hi)`` through ``[b_r, b_{r+1})`` from the frontier buffers (the
    corrected rows carry their corrected input, the others their approximate one; all of them see
    the corrected K/V and side-buffer rows as context -- the staged schedule's semantics).
    ...
    L = num_layers(self)
    bounds = stage_bounds(L, g)
    b = bounds[r]
    nxt = bounds[r + 1] if r + 1 < g else None
    assert (b == L) == bool(final), (r, g, b, L, final)
    rows = torch.arange(sb.lo, sb.hi, dtype=torch.int64)
    n1 = sb.n - 1
    log = []
    ...
    info["stage"] = {"round": r, "rounds": g, "bounds": bounds, "steps": log,
                     "t_total_ms": sum(x["t_ms"] for x in log)}
...
def _correct_sub(self: GPUModelRunner, req_id: str, sb: SideBuffer, positions: torch.Tensor,
    ...
    # paged_kv_indptr, flashinfer.py:921) size their persistent buffers by max_num_seqs
    ...
    for gid, group in enumerate(self.kv_cache_config.kv_cache_groups):
        ...
        blk = row.unsqueeze(0).expand(P, -1).contiguous()
        cm = CommonAttentionMetadata(
            query_start_loc=qsl_cpu.to(dev),
            query_start_loc_cpu=qsl_cpu,
            seq_lens=seq_cpu.to(dev),
            _seq_lens_cpu=seq_cpu,
            _num_computed_tokens_cpu=pos_cpu.to(torch.int32),
            seq_lens_cpu_upper_bound=seq_cpu,
            num_reqs=P,
            num_actual_tokens=P,
            max_query_len=1,
            max_seq_len=int(pos_cpu[-1]) + 1,
            block_table_tensor=blk,
            slot_mapping=slots,
            causal=True,
            is_prefilling=torch.zeros(P, dtype=torch.bool),
            positions=positions,
        )
