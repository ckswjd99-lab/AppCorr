...
class Glm46VAxis(QwenVLStreamingAxis):
    ...
    # Fallbacks for `_image_token_run` when the config omits the sentinel ids. Class attributes
    # rather than module constants so the GLM-5.3-Flash subclass (different ids, same prompt
    # shape) inherits the check instead of re-typing it.
    ...
    def _image_token_run(self, input_ids: torch.Tensor) -> Tuple[int, int]:
        ...
        ids = input_ids[0]
        seq = int(ids.shape[0])
        start_id = int(getattr(self.cfg, "image_start_token_id", None) or self.IMAGE_START_ID)
        end_id = int(getattr(self.cfg, "image_end_token_id", None) or self.IMAGE_END_ID)
