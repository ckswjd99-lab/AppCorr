...
class ApproxCorrectQwen25VLVisionBlock(nn.Module):
    ...
    def approx(self, x: torch.Tensor, cu_seqlens: torch.Tensor, position_embeddings, cache_feature: Dict[str, Any], tag: str):
        x_attn, cache_feature = self.attn.approx(self.norm1(x), cu_seqlens, position_embeddings, cache_feature, tag)
    ...
    def correct(
        ...
        x_attn_sel, cache_feature = self.attn.correct(
            x_norm_sel, token_idx, cu_seqlens, position_embeddings_sel, cache_feature, tag
        )
