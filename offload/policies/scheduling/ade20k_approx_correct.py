...
class ADE20KApproxCorrectPolicy(ISchedulingPolicy):
    """ADE20K segmentor scheduling with approx-then-correct.
    ...
    def decide(
        ...
        **kwargs,
        ...
        if current_group == 0:
            # Phase 1: Base layer → start approx
            ...
            # Phase 2: Residual → load full-res → prepare → correct + head + send
            instructions = [
                Instruction(OpType.LOAD_INPUT),
                Instruction(OpType.PREPARE_TOKENS),
            ]
