...
def _quantize_block_copies(
    blocks: nn.ModuleList,
    precision: str,
    device: torch.device,
) -> tuple[nn.ModuleList | None, str | None]:
    """Deep-copy every block in `blocks` and convert its 5 eligible Linear layers
    (attn.qkv/proj, mlp.w1/w2/w3) to FP8 or FP4 via TorchAO. Returns
    (quantized_blocks, None) on success, or (None, reason) if unavailable/failed. Does not
    torch.compile anything -- callers decide whether/how to dispatch into the result.
    """
    if device.type != "cuda" or not torch.cuda.is_available():
        return None, f"{precision.upper()} requires a CUDA device"

    min_capability = (8, 9) if precision == "fp8" else (10, 0)
    capability = torch.cuda.get_device_capability(device)
    if capability < min_capability:
        return None, (
            f"compute capability {capability[0]}.{capability[1]} is below "
            f"{min_capability[0]}.{min_capability[1]}"
        )
    ...
    try:
        if precision == "fp8":
            ...
            eligible_fn = _eligible_linears
            converted_type_name = "Float8Tensor"
        else:
            from torchao.prototype.mx_formats import NVFP4DynamicActivationNVFP4WeightConfig
            from torchao.quantization import quantize_

            # use_triton_kernel=False: the Triton NVFP4 kernel requires the MSLK package, not
            # installed here; eager dispatch is correctness-equivalent and latency isn't the
            # target. use_dynamic_per_tensor_scale=True: the accurate mode.
            quant_config = NVFP4DynamicActivationNVFP4WeightConfig(
                use_triton_kernel=False,
                use_dynamic_per_tensor_scale=True,
            )
            eligible_fn = _eligible_fp4_linears
            converted_type_name = "NVFP4Tensor"
    except ImportError as exc:
        return None, f"TorchAO support for {precision} is required ({exc})"

    quantized_blocks = nn.ModuleList()
    quantized_count = 0
    try:
        for block in blocks:
            q_block = (
            ...
            converted = [
                name
                for name, module in q_block.named_modules()
                if name in eligible_names
                and type(module.weight).__name__ == converted_type_name
            ]
            if len(converted) != len(eligible_names):
                raise RuntimeError(
                    f"TorchAO skipped one or more requested {precision.upper()} weights: "
    ...
    expected_count = len(blocks) * len(_LOW_PRECISION_LINEAR_SUFFIXES)
    if quantized_count != expected_count:
        return None, f"quantized {quantized_count} Linear layers, expected {expected_count}"

    return quantized_blocks, None
...
class DINOv3FullPrecisionController:
    """Quantize the STOCK `.forward()` path used by `full_inference` (the "ceiling" / no-
    approximation baseline) -- for comparison against DINOv3CorrectPrecisionController: does low
    precision cost accuracy generally, or does it only stay lossless on the correction path
    specifically because correction only ever recomputes a small selected-token subset each round?

    Unlike the approx/correct controllers, `full_inference` calls into vendored library code
    (`dinov3.eval.segmentation.inference.make_inference`) with no per-layer hook point available --
    it just calls each block's plain `.forward()` internally via `nn.Module.__call__`. So instead of
    a `run_block()`-style dispatch, this controller works by IN-PLACE swapping
    `vit_backbone.blocks` for quantized clones once at load time (`apply_to_backbone`); every
    subsequent call into the vendored inference code then transparently runs the quantized blocks'
    `.forward()`. This is only valid for configs that never call approx_forward/correct_forward on
    the same backbone instance (e.g. ADE20KSequential's full-baseline path, which never invokes
    them) -- the swap is permanent for the executor's lifetime, matching this repo's convention of
    spinning up a fresh server process per config for evaluation.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.precision = precision
        self.device = device
        self.quantized_blocks: nn.ModuleList | None = None
        self._unavailable_reason: str | None = None

        if precision in {"fp8", "fp4"}:
            self.quantized_blocks, self._unavailable_reason = _quantize_block_copies(
                blocks, precision, device
            )
            if self.quantized_blocks is None:
                raise RuntimeError(
                    f"full_precision='{precision}' is unavailable: {self._unavailable_reason}"
                )
            print(
                f"[{precision.upper()}-full] Prepared "
                f"{len(self.quantized_blocks) * len(_LOW_PRECISION_LINEAR_SUFFIXES)} "
                f"full-inference Linear weights across {len(self.quantized_blocks)} blocks."
            )
    ...
    def from_config(
        ...
        config: Any,
        device: torch.device,
    ) -> "DINOv3FullPrecisionController":
        return cls(blocks, precision=config.full_precision, device=device)

    def apply_to_backbone(self, backbone: nn.Module) -> None:
        """In-place swap `backbone.blocks` with the quantized clones, if configured."""
        if self.quantized_blocks is not None:
            backbone.blocks = self.quantized_blocks
