...
def sampled_hw(proc, img):
    """What the model really samples: the processor's own grid, in pixels."""
    enc = proc(text=["<|vision_start|><|image_pad|><|vision_end|>"], images=[img],
               return_tensors="pt")
    g = enc["image_grid_thw"][0].tolist()
    p = proc.image_processor.patch_size
    return g[1] * p, g[2] * p          # (h, w)


def l2_from_native(img: Image.Image, level: int, proc) -> Image.Image:
    """Degrade content by 2**level relative to whichever of {native, sampled} binds, per axis."""
    w, h = img.size
    sh, sw = sampled_hw(proc, img)
...
def main():
    ...
    for name, w, h in CASES:
        ...
        sh, sw = sampled_hw(proc, img)
        out = l2_from_native(img, LEVEL, proc)
        ...
        ok &= good
        print(f"  {'PASS' if good else 'FAIL'}  {name:<15} {w}x{h} sampled {sw}x{sh} "
              f"keep {want[0]}x{want[1]}  blur {bw:.2f}/{bh:.2f} (want {exp_w:.2f}/{exp_h:.2f})")
