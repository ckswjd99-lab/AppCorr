...
class COCOCustom(COCO):
    ...
    def loadRes(self, resFile):
        ...
        if type(resFile) == str:
            ...
            # LOCAL PATCH (AppCorr): copy, do not alias.
            #
            # `CGF1Evaluator.evaluate` hands the SAME per-image prediction list to loadRes once per
            # ground truth -- three times in the Gold oracle setting. The `"segmentation" in anns[0]`
            # branch below mutates those dicts, adding `bbox` from `maskUtils.toBbox`. So annotator
            # a takes the segmentation branch while b and c, now seeing a `bbox` key, take the bbox
            # branch instead and derive `area` from the box rather than the mask -- the three
            # annotators end up scored on different rules.
            #
            # On numpy >= 1.25 it does not even get that far: the guard below compares an ndarray of
            # shape (4,) against `[]` and raises. Copying restores the intended semantics for all
            # three passes and removes the crash as a side effect.
