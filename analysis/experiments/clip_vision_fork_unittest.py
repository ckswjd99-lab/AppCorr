...
def main():
    ...
    with torch.no_grad():
        ...
        cache_a = {}
        approx_embeds, cache_a = tower.approx_forward(pixel_values, cache_a, tag_prefix="a")
        ...
        cache_b = {}
        _, cache_b = tower.approx_forward(blurred, cache_b, tag_prefix="b")
        all_patch_idx = torch.arange(num_patches, device=device)
        correct_all_embeds, cache_b = tower.correct_forward(pixel_values, all_patch_idx, cache_b, tag_prefix="b")
        ...
        cache_c = {}
        _, cache_c = tower.approx_forward(blurred, cache_c, tag_prefix="c")
        half_patch_idx = torch.arange(0, num_patches, 2, device=device)  # every other patch
        correct_half_embeds, cache_c = tower.correct_forward(pixel_values, half_patch_idx, cache_c, tag_prefix="c")
    ...
    print(f"\n[unittest] RESULT: (a) approx-only {'PASS' if ok_a else 'FAIL'}, "
          f"(b) correct-all {'PASS' if ok_b else 'FAIL'}")
