...
def main():
    ...
    ap.add_argument("--llm-schedule", choices=["streaming", "interleaved", "interleaved_staged"],
                    default="streaming",
                    help="how the LLM consumes the bands (docs/memo/vllm_interleaved_design.md). "
                         ...
                         "layer band -- memo §7.11). Names the progressive arm's rows "
            ...
            if arm == "streaming":
                ...
                extra = {"corrected_groups": int(st["corrected_groups"]),
                         "chunks": ([list(c) for c in st["chunks"]]
                                    if str(st.get("llm_schedule", "")).startswith("interleaved")
                                    else len(st["chunks"])),
