...
#
# PERSIST=false reproduces the pre-fix numbers, when a corrected increment was never written back to
# `blocks_out_sum` and every round rebuilt earlier groups from the stale approximate one. Rounds cost
# 12.6pp of recovery then; with the fix (now the default) they cost ~0. Keep both arms runnable --
# the whole point of the measurement is the difference between them.
set -u
cd /NHNHOME/share/cjpark/AppCorr
NR=${NR:-20}
PERSIST=${PERSIST:-true}
TAG=$([ "$PERSIST" = "true" ] && echo persist || echo prefix)
for c in co3d_full co3d_approx_only co3d_appcorr co3d_interleaved co3d_il_frame4 co3d_il_spatial8 co3d_il_spatial4; do
  ...
  CUDA_VISIBLE_DEVICES=${GPU:-0} PYTHONUNBUFFERED=1 APPCORR_RESULT_TIMEOUT=1800 timeout 3600 \
    ...
    --set appcorr_kwargs.persist_correction_residual=$PERSIST \
